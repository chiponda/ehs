...
const getForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    api
        ...
        .then((res) => {
            if (res.status === 201) alert("Risk has beed recorded successfully");
            else alert("Failed to record Risk .");
            getForm();
        })
        .catch((err) => alert(err));
};
    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl p-8 w-full w-full  max-w-lg mx-auto">
        <div className="flex items-center justify-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-blue-600">Record Risk </h1>
        </div>
        <form onSubmit={handleSubmit} className="space-y-10 space-x-50 w-full" >
          ...
          <div>
            <label htmlFor="mitigation" className="block text-l font-medium text-gray-700  ">Mitigation measures you sugggest</label>
            <textarea
